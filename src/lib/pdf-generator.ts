...
/**
 * Draw a 3D cargo box using simple rectangles (avoids jsPDF polygon issues)
 */
function draw3DCargoBox(doc: jsPDF, x: number, y: number, length: number, _width: number, height: number, color: string) {
  const r = parseInt(color.slice(1, 3), 16)
  const g = parseInt(color.slice(3, 5), 16)
  const b = parseInt(color.slice(5, 7), 16)

  // Main face (front rectangle)
  doc.setFillColor(r, g, b)
  doc.setDrawColor(Math.floor(r * 0.7), Math.floor(g * 0.7), Math.floor(b * 0.7))
  doc.rect(x, y, length, height, 'FD')

  // Add a subtle highlight at top to give 3D effect
  doc.setFillColor(Math.min(r + 30, 255), Math.min(g + 30, 255), Math.min(b + 30, 255))
  doc.rect(x, y, length, 3, 'F')

  // Add shadow at bottom for depth
  doc.setFillColor(Math.floor(r * 0.6), Math.floor(g * 0.6), Math.floor(b * 0.6))
  doc.rect(x, y + height - 2, length, 2, 'F')
}
